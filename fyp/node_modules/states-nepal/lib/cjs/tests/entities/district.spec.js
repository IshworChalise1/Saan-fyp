"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const district_1 = require("../../src/entities/district");
const utils_1 = require("../../src/utils");
const APP_LANG = 'np';
const _district = new district_1.District(APP_LANG);
describe('Test District entities', () => {
    it('should test number of district in Nepal', () => {
        expect(_district.allDistricts().length).toBe(77);
    });
    it('should test find district by Id', () => {
        const correctRange = (0, utils_1.range)(1, 77);
        correctRange.forEach(id => {
            const item = _district.find(id);
            expect(item).not.toBeNull();
        });
        const incorrectRange = (0, utils_1.range)(78, 154);
        incorrectRange.forEach(id => {
            const item = _district.find(id);
            expect(item).toBeNull();
        });
    });
    it('should return the largest district by area', () => {
        const largest = _district.largest();
        expect(largest).toMatchObject({ id: 61, province_id: 6 });
    });
    it('should return the smallest district by area', () => {
        const smallest = _district.smallest();
        expect(smallest).toMatchObject({ id: 23, province_id: 3 });
    });
    it('should get districts with municipalities', () => {
        const districtsWithMunicipalities = _district.getDistrictsWithMunicipalities();
        districtsWithMunicipalities.map(item => {
            var _a;
            expect(item.municipalities).toBeDefined();
            expect((_a = item.municipalities) === null || _a === void 0 ? void 0 : _a.length).toBeGreaterThanOrEqual(1);
        });
    });
    it('should search districts that match with given key', () => {
        const keywords = [
            'id',
            'province_id',
            'name',
            'area_sq_km',
            'website',
            'headquarter',
        ];
        const districts = _district.allDistricts();
        for (const key of keywords) {
            for (const value of districts) {
                const items = _district.search(key, value[key]);
                expect(items.length).toBeGreaterThanOrEqual(1);
            }
        }
    });
});
//# sourceMappingURL=district.spec.js.map